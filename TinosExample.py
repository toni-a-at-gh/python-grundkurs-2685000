...
parser = argparse.ArgumentParser(
    description="Dies ist eine einfache Taschenrechner-App"
)
...
parser.add_argument(
    "Operand", help="Operand ('+', '-', '*' oder '/')", choices=["+", "-", "*", "/"]
)

# Definieren von optionalen Parametern:
parser.add_argument(
    "--GleichungSchreiben",
    help="True: Gleichung wird ausgegeben. False: Nur Ergebnis wird ausgegeben.",
    type=bool,
)
...
print(
    "Die folgenden Argumente wurden eingegeben: "
    + str(args.linkerOperator)
    + " "
    + str(args.rechterOperator)
    + " "
    + str(args.Operand)
    + " "
    + str(args.GleichungSchreiben)
)
...
if args.Operand == "+":
    if bool(args.GleichungSchreiben) == True:
        print(
            "Das Ergebnis lautet: "
            + str(float(args.linkerOperator))
            + " + "
            + str(float(args.rechterOperator))
            + " = "
            + str(float(args.linkerOperator) + float(args.rechterOperator))
        )
    else:
        print(
            "Das Ergebnis lautet "
